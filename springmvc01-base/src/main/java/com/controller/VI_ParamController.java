...
@Controller
public class VI_ParamController {

    /**
     * 在SpringMVC中接收请求参数使用的处理请求发法的形参
     * 自带形参
     * 1.HttpServletRequest 请求对象
     * 2.HttpServletResponse 响应对象
     * 3.HttpSession 会话对象
     * 4.Model: 由Spring提供的一个API接口，作用是替代原生request域（了解）
     */
     ...
     * 接受简单类型
     *
     * SpringMVVC通过处理请求方法的形参就可以直接接收请求参数，并且能够自动实现数据的类型转换
     * 默认日志格式：yyy/MM/dd
     *
     * 要求：请求参数名与形参的名称必须保持一致
     *
     * @DateTimeFormat 作用：将请求参数中日期字符串按照指定格式进行转换
     */
